/*
 * Put this file in the same folder as ebook_index.html.
 * Keep the variable name Storys so ebook_script.js can load it.
 */

const Storys = [
	{
    //Paradox: The Haunting Of The Heddal Stave Church
    Name: "Paradox: The Haunting Of The Heddal Stave Church",
    Genres: ["Fantasy", "Horror"],
    Description: "",
    Logo: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/logo_black.svg",
    Port: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/background.png",
    Wallpaper: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/wallpaper.png",
    Pdfstory: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/pdf.pdf",
    Epub: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/.epub",
    Ebookcover: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/bookcover.png",
    Gif: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/bookcover.png",
    Series: "Paradox",
    Worddoc: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/ebook.docx",
	model: "assets/stories/paradox_the_haunting_of_the_heddal_stave_church/bookmodel.glb",
    StoryType: "ShortStory",
    ReleaseDate: "2098-01-23"
  },
	{
    //Anatomy Of A Mans Demons
    Name: "Anatomy Of A Mans Demons",
    Genres: ["Fantasy", "Horror"],
    Description: "",
    Logo: "assets/stories/anatomy_of_a_mans_demons/logo_black.svg",
    Port: "assets/stories/anatomy_of_a_mans_demons/background.png",
    Wallpaper: "assets/stories/anatomy_of_a_mans_demons/wallpaper.png",
    Pdfstory: "assets/stories/anatomy_of_a_mans_demons/pdf.pdf",
    Epub: "assets/stories/anatomy_of_a_mans_demons/.epub",
    Ebookcover: "assets/stories/anatomy_of_a_mans_demons/bookcover.png",
    Gif: "assets/stories/anatomy_of_a_mans_demons/bookcover.png",
    Series: "Elseworlds",
    Worddoc: "assets/stories/anatomy_of_a_mans_demons/ebook.docx",
	model: "assets/stories/anatomy_of_a_mans_demons/bookmodel.glb",
    StoryType: "ShortStory",
    ReleaseDate: "2098-01-23"
  },
	{
    //The Etenral Ones: The Impossible Choice
    Name: "The Etenral Ones: The Impossible Choice",
    Genres: ["Fantasy", "Drama"],
    Description: "",
    Logo: "assets/stories/the_ertenral_ones_the_impossible_choice/logo_black.svg",
    Port: "assets/stories/the_ertenral_ones_the_impossible_choice/background.png",
    Wallpaper: "assets/stories/the_ertenral_ones_the_impossible_choice/wallpaper.png",
    Pdfstory: "assets/stories/the_ertenral_ones_the_impossible_choice/pdf.pdf",
    Epub: "assets/stories/the_ertenral_ones_the_impossible_choice/.epub",
    Ebookcover: "assets/stories/the_ertenral_ones_the_impossible_choice/bookcover.png",
    Gif: "assets/stories/the_ertenral_ones_the_impossible_choice/bookcover.png",
    Series: "The Eternal Ones",
    Worddoc: "assets/stories/the_ertenral_ones_the_impossible_choice/ebook.docx",
	model: "assets/stories/the_ertenral_ones_the_impossible_choice/bookmodel.glb",
    StoryType: "ShortStory",
    ReleaseDate: "2098-01-23"
  },
   {
    //The Eternal Ones: Cruelty Of Compassion
    Name: "The Eternal Ones: Cruelty Of Compassion",
    Genres: ["Fantasy", "Drama"],
    Description: "",
    Logo: "assets/stories/the_eternal_ones_cruelty_of_compassion/logo_black.svg",
    Port: "assets/stories/the_eternal_ones_cruelty_of_compassion/background.png",
    Wallpaper: "assets/stories/the_eternal_ones_cruelty_of_compassion/wallpaper.png",
    Pdfstory: "assets/stories/the_eternal_ones_cruelty_of_compassion/pdf.pdf",
    Epub: "assets/stories/the_eternal_ones_cruelty_of_compassion/The Eternal Ones_ Cruelty Of Compassion - Kyle Evan Young.epub",
    Ebookcover: "assets/stories/the_eternal_ones_cruelty_of_compassion/bookcover.png",
    Gif: "assets/stories/the_eternal_ones_cruelty_of_compassion/bookcover.png",
    Series: "The Eternal Ones",
    Worddoc: "assets/stories/the_eternal_ones_cruelty_of_compassion/ebook.docx",
	model: "assets/stories/the_eternal_ones_cruelty_of_compassion/bookmodel.glb",
    StoryType: "ShortStory",
    ReleaseDate: "2098-01-23"
  },
	{
    //The Terribly Long Horrible No Good Night At Memory Lane
    Name: "The Terribly Long Horrible No Good Night At Memory Lane",
    Genres: ["Action", "Sci-Fi"],
    Description: "",
    Logo: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/logo_black.svg",
    Port: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/background.png",
    Wallpaper: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/wallpaper.png",
    Pdfstory: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/pdf.pdf",
    Epub: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/.epub",
    Ebookcover: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/bookcover.png",
    Gif: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/bookcover.png",
    Series: "Elseworlds",
    Worddoc: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/ebook.docx",
	model: "assets/stories/the_terribly_long_horrible_no_good_night_at_memory_lane/bookmodel.glb",
    StoryType: "ShortStory",
    ReleaseDate: "2098-01-23"
  },
  {
    // THe Etenral Ones: The City Of Roanina
    Name: "The City Of Roanina",
    Genres: ["Fantasy", "Sci-Fi", "Drama"],
    Description: "Some of the Ertenral Ones make a deal to watch over a village, giving a wish to a person of a true heart.",
    Logo: "assets/stories/the_ertenral_ones_the_city_of_roanina/logo_black.svg",
    Port: "assets/stories/the_ertenral_ones_the_city_of_roanina/background.png",
    Wallpaper: "assets/stories/the_ertenral_ones_the_city_of_roanina/wallpaper.png",
    Pdfstory: "assets/stories/the_ertenral_ones_the_city_of_roanina/pdf.pdf",
    Epub: "assets/stories/the_ertenral_ones_the_city_of_roanina/Etenral Ones_ The City Of Roanina, The - Kyle Young.epub",
    Ebookcover: "assets/stories/the_ertenral_ones_the_city_of_roanina/bookcover.png",
    Gif: "assets/stories/the_ertenral_ones_the_city_of_roanina/bookcover.png",
    Series: "The Eternal Ones",
    Worddoc: "assets/stories/the_ertenral_ones_the_city_of_roanina/ebook.docx",
	model: "assets/stories/the_ertenral_ones_the_city_of_roanina/bookmodel.glb",
    StoryType: "ShortStory",
    ReleaseDate: "2098-01-23"
  },
  
  
  
  {
    //The Secret Of Owen Taylor A Dreamer Story
    Name: "The Secret Of Owen Taylor A Dreamer Story",
    Genres: ["Action", "Superhero", "Adventure", "Drama"],
    Description: "A story that follows Owen, Tessa, and Sebastian as they sneak into Elias Industries and discover a dangerous machine that alters reality and gives Tessa powers connected to Owen’s alien DNA.",
    Logo: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/logo_black.svg",
    Port: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/background.png",
    Wallpaper: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/wallpaper.png",
    Pdfstory: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/pdf.pdf",
    Epub: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/The Secret Of Owen Taylor_ A Dreamer Story - Kyle Young.epub",
    Ebookcover: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/bookcover.png",
    Worddoc: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/ebook.docx",
    Gif: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/bookcover.png",
	model: "assets/stories/the_secret_of_owen_taylor_a_dreamer_story/bookmodel.glb",
    Series: "Superhero",
    StoryType: "Novelette",
    ReleaseDate: "2025-10-24"
  },
  {
    //The Old Man At The Bar
    Name: "The Old Man At The A Bar,",
    Genres: ["Drama", "Grief Fiction"],
    Description: "On the worst night of his life, James walks into The Old Man at the Bar carrying a grief too heavy to survive. His wife, his children, and his friends are gone after a plane crash with no survivors, and all he has left is a wedding ring, a whiskey, and memories he can barely face.",
    Logo: "assets/stories/the_old_man_at_the_bar/logo_black.svg",
    Port: "assets/stories/the_old_man_at_the_bar/background.png",
    Wallpaper: "assets/stories/the_old_man_at_the_bar/wallpaper.png",
    Pdfstory: "assets/stories/the_old_man_at_the_bar/pdf.pdf",
    Epub: "assets/stories/the_old_man_at_the_bar/The Old Man At The Bar - Kyle Young.epub",
    Ebookcover: "assets/stories/the_old_man_at_the_bar/bookcover.png",
    Gif: "assets/stories/the_old_man_at_the_bar/bookcover.png",
    Series: "Elseworlds",
    Worddoc: "assets/stories/the_old_man_at_the_bar/ebook.docx",
	model: "assets/stories/the_old_man_at_the_bar/bookmodel.glb",
    StoryType: "ShortStory",
    ReleaseDate: "2025-10-24"
  },
  {
    //The Last Prank Of Tomist
    Name: "The Last Prank Of Tomist",
    Genres: ["Kids", "Comedy", "Sci-Fi", "Coming Of Age"],
    Description: "After a strange crash on a dark road, twelve-year-old Leo discovers something impossible: a glowing alien ship, two panicked blue aliens, and a mysterious watch that may be the key to fixing reality itself.",
    Logo: "assets/stories/the_last_prank_of_tomist/logo_black.svg",
    Port: "assets/stories/the_last_prank_of_tomist/background.png",
    Wallpaper: "assets/stories/the_last_prank_of_tomist/wallpaper.png",
    Pdfstory: "assets/stories/the_last_prank_of_tomist/pdf.pdf",
    Epub: "assets/stories/the_last_prank_of_tomist/The Last Prank of Tomist - Kyle Young.epub",
    Ebookcover: "assets/stories/the_last_prank_of_tomist/bookcover.png",
    Gif: "assets/stories/the_last_prank_of_tomist/bookcover.png",
    Series: "Elseworlds",
    Worddoc: "assets/stories/the_last_prank_of_tomist/ebook.docx",
	model: "assets/stories/the_last_prank_of_tomist/bookmodel.glb",
    StoryType: "Novelette",
    ReleaseDate: "2025-10-24"
  },
  {
    //Love Love And Lost Even More
    Name: "Love Love And Lost Even More",
    Genres: ["Drama", "Crime", "Tragedy"],
    Description: "On the night of Liam and Sonia’s twentieth anniversary, everything feels familiar: friends, laughter, wine, and old memories. But at 4 a.m., Hannah arrives at their door bloodied, terrified, and broken - carrying an accusation that will tear the group apart.",
    Logo: "assets/stories/love_love_and_lost_even_more/logo_black.svg",
    Port: "assets/stories/love_love_and_lost_even_more/background.png",
    Wallpaper: "assets/stories/love_love_and_lost_even_more/wallpaper.png",
    Pdfstory: "assets/stories/love_love_and_lost_even_more/pdf.pdf",
    Epub: "assets/stories/love_love_and_lost_even_more/Love, Lost And Lost Evenmore - Kyle Young.epub",
    Ebookcover: "assets/stories/love_love_and_lost_even_more/bookcover.png",
    Worddoc: "assets/stories/love_love_and_lost_even_more/ebook.docx",
    Gif: "assets/stories/love_love_and_lost_even_more/bookcover.png",
	model: "assets/stories/love_love_and_lost_even_more/bookmodel.glb",
    Series: "Elseworlds",
    StoryType: "ShortStory",
    ReleaseDate: "2025-08-23"
  },
   {
    // The Forgotten Kingdom
    Name: "The Forgotten Kingdom",
    Genres: ["Fantasy", "Adventure", "Drama"],
    Description: "The Forgotten Kingdom is a fantasy adventure about three heroes who enter a lost underwater kingdom in search of healing, redemption, and forgotten power. Along the way, they must solve magical trials, uncover buried memories, and confront the dangerous Mindseer at the heart of the legend.",
    Logo: "assets/stories/the_forgotten_kingdom/logo_black.svg",
    Port: "assets/stories/the_forgotten_kingdom/background.png",
    Wallpaper: "assets/stories/the_forgotten_kingdom/wallpaper.png",
    Pdfstory: "assets/stories/the_forgotten_kingdom/pdf.pdf",
    Epub: "assets/stories/the_forgotten_kingdom/The Forgotten Kingdom - Kyle Young.epub",
    Ebookcover: "assets/stories/the_forgotten_kingdom/bookcover.png",
    Worddoc: "assets/stories/the_forgotten_kingdom/ebook.docx",
    Gif: "assets/stories/the_forgotten_kingdom/bookcover.png",
	model: "assets/stories/the_forgotten_kingdom/bookmodel.glb",
    Series: "Elseworlds",
    StoryType: "ShortStory",
    ReleaseDate: "2025-09-23"
  },
  {
    //The Interrogation
    Name: "The Interrogation",
    Genres: ["Mystery", "Psychological", "Suspense"],
    Description: "A man trapped in what appears to be a police interrogation, where Officer Lois accuses him of being connected to the disappearance of several women.",
    Logo: "assets/stories/the_interrogation/logo_black.svg",
    Port: "assets/stories/the_interrogation/background.png",
    Wallpaper: "assets/stories/the_interrogation/wallpaper.png",
    Pdfstory: "assets/stories/the_interrogation/pdf.pdf",
    Epub: "assets/stories/the_interrogation/The Interrogation - Kyle Young.epub",
    Ebookcover: "assets/stories/the_interrogation/bookcover.png",
    Worddoc: "assets/stories/the_interrogation/ebook.docx",
    Gif: "assets/stories/the_interrogation/bookcover.png",
	model: "assets/stories/the_interrogation/bookmodel.glb",
    Series: "Elseworlds",
    StoryType: "FlashFiction",
    ReleaseDate: "2025-08-23"
  },
  {
    //The Puzzle And The Dragon
    Name: "The Puzzle And The Dragon",
    Genres: ["Fantasy", "Kids"],
    Description: "A boy who discovers an enchanted puzzle in a magical forest, believing that solving it will grant him a life-changing wish.",
    Logo: "assets/stories/the_puzzle_and_the_dragon/logo_black.svg",
    Port: "assets/stories/the_puzzle_and_the_dragon/background.png",
    Wallpaper: "assets/stories/the_puzzle_and_the_dragon/wallpaper.png",
    Pdfstory: "assets/stories/the_puzzle_and_the_dragon/pdf.pdf",
    Epub: "assets/stories/the_puzzle_and_the_dragon/The Puzzle And The Dragon - Kyle Young.epub",
    Ebookcover: "assets/stories/the_puzzle_and_the_dragon/bookcover.png",
    Gif: "assets/stories/the_puzzle_and_the_dragon/bookcover.png",
    Series: "Elseworlds",
    Worddoc: "assets/stories/the_puzzle_and_the_dragon/ebook.docx",
    model: "assets/stories/the_puzzle_and_the_dragon/bookmodel.glb",
    StoryType: "FlashFiction",
    ReleaseDate: "2025-07-23"
  },
  {
    //Paradox The Traveller("Chapter 1")
    Name: "Paradox The Traveller(Chapter 1)",
    Genres: ["Fantasy", "Adventure"],
    Description: "In a fusion of destinies and mystical quests, Noah, known as The Traveler, embarks on a perilous journey through time in search of magical elements essential for mastering the forces of creation.",
    Logo: "assets/stories/paradox_the_traveller/Chapter 1 Echoes of a Kingdom white",
    Port: "assets/stories/paradox_the_traveller/background.png",
    Wallpaper: "assets/stories/paradox_the_traveller/wallpaper.png",
    Pdfstory: "assets/stories/paradox_the_traveller/pdf.pdf",
    Epub: "assets/stories/paradox_the_traveller/Paradox The Traveller Echoes Of A Kingdom Chapter 1 - Kyle Young",
    Ebookcover: "assets/stories/paradox_the_traveller/bookcover.png",
    Gif: "assets/stories/paradox_the_traveller/bookcover.png",
    Series: "Paradox",
    Worddoc: "assets/stories/paradox_the_traveller/ebook.docx",
    model: "assets/stories/paradox_the_traveller/bookmodel.glb",
    StoryType: "Novel",
    ReleaseDate: "2025-03-25"
  }
];
